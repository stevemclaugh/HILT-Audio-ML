...
Try adapting the examples above to transcode a few files in our `sample_audio` set.
...
The `-ss` option specifies start time and `-t` is the length of our new excerpt. In this case we’re creating a 90-second clip beginning 10 minutes, 11 seconds into the film.  This may take a few minutes.
